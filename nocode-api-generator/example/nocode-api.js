 ...
 *
 * 方式一：通用函数（传入数据源和表名）
 *   import { list, get, add, update, del, batchDelete, customQuery } from '@/api/nocode'
 *
 *   // 查询列表
 *   list('business', 'des_chuhuodan9_1', { page: 1, size: 10 }, 'hrp_base')
 *
 *   // 查询单条
 *   get('business', 'des_chuhuodan9_1', '99f96301676b01036ad6a8273567d86a', 'hrp_base')
 *
 *   // 新增
 *   add('business', 'des_chuhuodan9_1', data, 'hrp_base')
 *
 *   // 更新
 *   update('business', 'des_chuhuodan9_1', id, data, 'hrp_base')
 *
 *   // 删除
 *   del('business', 'des_chuhuodan9_1', id, 'hrp_base')
 *
 * 方式二：为特定表创建快捷函数
 *   import { createTableApi } from '@/api/nocode'
 *
 *   const chuhuoApi = createTableApi('business', 'des_chuhuodan9_1', 'hrp_base')
 *   chuhuoApi.list({ page: 1, size: 10 })
 *   chuhuoApi.get(id)
 *
 * 方式三：自定义快捷函数（参考下方示例）
...
// Nocode API 基础路径（配合 VUE_APP_NOCODE_API 环境变量使用）
// 开发环境: VUE_APP_NOCODE_API = /dev-api
// 生产环境: VUE_APP_NOCODE_API = /prod-api
// 最终路径: /dev-api/nocode-admin/api/{datasource}/{table}
const NOCODE_BASE_URL = '/nocode-admin/api'
...
export function list(datasource, table, query, schema) {
  ...
  return request({
    url: NOCODE_BASE_URL + '/' + datasource + '/' + table,
    method: 'get',
    params
  })
}
...
export function get(datasource, table, id, schema) {
  ...
  return request({
    url: NOCODE_BASE_URL + '/' + datasource + '/' + table + '/' + id,
    method: 'get',
    params
  })
}
...
export function batchAdd(datasource, table, dataList, schema) {
  ...
  return request({
    url: NOCODE_BASE_URL + '/' + datasource + '/' + table + '/batch',
    method: 'post',
    params: params,
    data: dataList
  })
...
export function customQuery(datasource, table, query) {
  return request({
    url: NOCODE_BASE_URL + '/' + datasource + '/' + table + '/query',
    method: 'post',
    data: query
  })
}
...
export function getDatasources() {
  return request({
    url: NOCODE_BASE_URL + '/datasources',
    method: 'get'
  })
}
...
export function getSchema(datasource, table, schema) {
  ...
  return request({
    url: NOCODE_BASE_URL + '/' + datasource + '/' + table + '/schema',
    method: 'get',
    params: params
  })
}
...
// 示例：获取所有数据源列表
// import { getDatasources } from '@/api/nocode'
// getDatasources().then(res => {
//   console.log(res.data) // [{name: "postgres_primary", jdbcUrl: "...", ...}, ...]
// })
